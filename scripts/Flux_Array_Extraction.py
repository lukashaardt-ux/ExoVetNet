...
# Flux array folder
if not USE_BLS:
    os.makedirs("data/fluxes_catalogv2",exist_ok=True)
else:
    os.makedirs("data/fluxes_bls", exist_ok=True)
...
for kepid in tqdm(df["kepid"].unique()):
    ...
    if len(star_names) > 0 and all(os.path.exists(f"data/fluxes_catalogv2/{n}.npz") for n in star_names):
        continue

    try:
        search_result = lk.search_lightcurve(f"KIC {kepid}", mission="Kepler", cadence="long")
    except Exception as e:  # noqa: BLE001
        print(f"Error occurred while searching for light curves for KIC {kepid}: {e}")
        continue
    ...
    while kepid in df_copy["kepid"].values:
        try:
            ...
            out_dir = "data/fluxes_bls" if USE_BLS else "data/fluxes_catalogv2"
            ...
            flux_arr_g = fill_nans(flux_arr_g)
            ...
            flux_arr_l = fill_nans(flux_arr_l)
            ...
            if not USE_BLS:
                np.savez(f"data/fluxes_catalogv2/{name}.npz",
                        global_view = flux_arr_g,
                        local_view = flux_arr_l,
                        label = label)
            else:
                np.savez(f"data/fluxes_bls/{name}.npz",
                        global_view = flux_arr_g,
                        local_view = flux_arr_l,
                        label = label)
